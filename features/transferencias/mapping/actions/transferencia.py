...
class Transferencia(BasePage):
    ...
    def preencher_campo_numero_conta(self, numero_conta):
        self.send_keys(TransferenciaObjs.CAMPO_NUMERO_CONTA, numero_conta)

    def preencher_campo_digito_conta(self, digito_conta):
        self.send_keys(TransferenciaObjs.CAMPO_DIGITO_CONTA, digito_conta)

    def preencher_campo_valor_transferencia(self, valor_transferencia):
        self.send_keys(TransferenciaObjs.CAMPO_VALOR_TRANSFERENCIA, valor_transferencia)

    def preencher_campo_descricao(self, descricao):
        self.send_keys(TransferenciaObjs.CAMPO_DESCRICAO, descricao)

    def realizar_transferencia(self):
        self.click(TransferenciaObjs.BOTAO_TRANSFERIR)

    def valida_texto_transferencia_realizada(self, texto):
        texto_retornado = self.retorna_texto(TransferenciaObjs.TEXTO_TRANSFERENCIA_REALIZADA)
